...
- **`printEBForm(layout)`** defaults to `'landscape1'` (A4 landscape, form on the left half sized as A5, right half blank, cut down the middle) — prints onto ordinary A4 paper, the one paper size every printer reliably supports. Direct-to-A5 printing (`'a5'` portrait, `'a5l'` landscape with a two-column `formHTMLLandscape` layout) was tried as the default first, but real hardware (a cheap HP LaserJet 103/107) could not handle a true A5 page correctly in either orientation — portrait printed rotated 90° onto A4, landscape overflowed onto a second page — because its driver doesn't properly support A5 as a distinct paper size. Both A5 modes are kept in the function as options for printers that *do* support A5, but `'landscape1'` is the one wired to the print button. Do not default back to an A5 mode without confirming the target printer actually handles A5 correctly first.
