...
- **`printEBForm(layout)`** defaults to `'a5l'` (A5 **landscape**, 210×148.5mm) — prints the label directly onto a single A5 sheet, no cutting, laid out as two columns (`formHTMLLandscape`: drug table left, signatures/patient-use rows right) since the page is now wide/short instead of narrow/tall. Uses the named `size: A5 landscape` CSS preset rather than raw custom mm dimensions — a raw custom page size is unreliably honored by real printer drivers and was observed printing rotated/onto the wrong physical paper; the named size + orientation combination is handled correctly far more consistently. `'a5'` (A5 portrait, single-column `formHTML`), `'landscape1'` (A4 landscape, form on the left half, right half blank, cut down the middle) and `'portrait1'` (full A4 portrait) still exist in the function for sites without A5 landscape paper on hand, but no UI button currently calls them — do not remove them without checking first.
