...
- **`printEBForm(layout)`** defaults to `'a5'` — prints the label directly onto a single A5 sheet (148.5×210mm, no cutting), requiring the printer fed with pre-cut/A5-tray paper. `'landscape1'` (A4 landscape, form on the left half, right half blank, cut down the middle) and `'portrait1'` (full A4 portrait) still exist in the function for sites without A5 paper on hand, but no UI button currently calls them — do not remove them without checking first.
